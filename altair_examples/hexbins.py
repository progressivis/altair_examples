...
(
    alt.Chart(source)
    .mark_point(size=size ** 2, shape=hexagon)
    .encode(
        x=alt.X(
            "xFeaturePos:Q",
            axis=alt.Axis(
                title="Month", grid=False, tickOpacity=0, domainOpacity=0
            ),
        ),
        y=alt.Y(
            "day(" + yField + "):O",
    ...
    .properties(
        ...
        height=size
        * yFeaturesCount
        * 1.7320508076,  # 1.7320508076 is approx. sin(60°)*2
    )
